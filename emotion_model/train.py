...
- Prepares data for training with max_length=128
"""
...
def main():
    ...
    print("=" * 70)
    print("Emotion Classification Model - Data Preparation")
    print("=" * 70)
    ...
    print("\n[3] Sample labels from training data:")
    sample_labels = train_df['labels'].head(10).tolist()
    for i, label in enumerate(sample_labels[:5], 1):
    ...
    multihot_label = sample_item['labels'].numpy()
    
    print(f"    Multi-hot label: {multihot_label}")
    print(f"    Active labels: {[label_map['id_to_label'][str(i)] for i, val in enumerate(multihot_label) if val > 0]}")
    ...
    training_args = TrainingArguments(
        ...
        metric_for_best_model="f1_macro",  # FIXED: Must match compute_metrics return key
